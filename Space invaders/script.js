    ...
    for(let i=0;i<armyRows; i++){
        for(let j=0;j<armyColumns;j++){
            ...
            if(soldier.status=='alive'){
                ...
                //check if game over by collison
                if(soldier.y > tankY){
                    gameRunning=false;
                }
            }
        ...
        for(let j=0; j<armyColumns; j++){
            ...
            )
            {
                ...
                if((aliveInvaders)%armyColumns==0 && armySpeed > 10){
                    ...
                    console.log("speed increse: " + armySpeed);
                }
            }
        }
    }
    //check if an invader is hit by the bullet
...
function drawStartScreen(){
    drawBlinker(function(){ drawScreen_line1("Space Invaders")}, function(){drawScreen_line2("Press Enter to Play")});
}
...
// returns a random interger, positive or negative
